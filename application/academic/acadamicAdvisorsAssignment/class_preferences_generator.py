...
class PreferenceGeneratorForIndividual:
    ...
    def append_preferences(self, preferences, random_order=False,
                           remove_duplicate_preferences=True):
        """添加偏好.
        ...
        # 删除重复的偏好
        if remove_duplicate_preferences:
            new_copy_preferences = []
            for item in copy_preferences:
                if isinstance(item, str):
                    found = False
                    if item in self._preference:
                        found = True
                        continue
                    for in_set in self._preference:
                        if isinstance(in_set, set):
                            if item in in_set:
                                found = True
                                continue
                    if not found:
                        new_copy_preferences.append(item)
                elif isinstance(item, set):
                    new_set = set()
                    for single_item in item:
                        found = False
                        if single_item in self._preference:
                            found = True
                            continue
                        for in_set in self._preference:
                            if isinstance(in_set, set):
                                if single_item in in_set:
                                    found = True
                                    continue
                        if not found:
                            new_set.add(single_item)
                    if len(new_set) > 0:
                        new_copy_preferences.append(new_set)
                else:
                    print('Wrong!')
                    raise Exception
            copy_preferences = new_copy_preferences
...
if __name__ == '__main__':
    ...
    if individuals_check:
        ...
        print(pg)
        pg.append_preferences(['w1', {'w3', 'w4'}]).append_preferences(['w6'])
        print(pg.to_dict())
