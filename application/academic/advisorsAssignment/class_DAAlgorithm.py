...
class Preference:
    ...
    def __len__(self):
        ...
        偏好序列表大小
        """
        return len(self._preference_list)

    @property
    def strict_preference(self):
        """返回严格偏好序.

        返回严格偏好序
        """
        strict_preference_list = []
        for item in self._preference_list:
            if isinstance(item, str):
                strict_preference_list.append(item)
            elif isinstance(item, set):
                strict_preference_list.append(random.choice(item))
            else:
                print('Wrong type!')
                raise Exception

        return strict_preference_list
    ...
    def values(self):
        ...
        偏好值
        """
        return list(self.preferences_rank)
