...
import pymongo
import pandas as pd
from dist.lib.class_mongodb import MongoDB, MonDatabase, MonCollection
...
class CityStatisticsDatabase:
    ...
    @property
    def period(self):
        found = self.conn.find().distinct('year')
        return pd.DataFrame(sorted(found))
...
class CityStatistics:
    ...
    def find(self,year=list(range(2000,2016)),variable=None,region=None,boundary='全市'):
        ...
        return self.city_stat_databse.find(query_str,
...
if __name__ == '__main__':
    ...
    city_database.variables.to_excel(r'E:\cyberspace\worklot\college\中国城市统计年鉴变量表.xlsx')
    ...
    print(result)
    #result.to_excel(r'E:\plutoese\project\lasting\datacollector\city statistics\output\citydata.xlsx')
