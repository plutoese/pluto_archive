        ...
        with xlrd.open_workbook(self.filename) as self.file:
            ...
            if sheet is None:
                ...
                if isinstance(self.sheet,int):
                    self.sheet.append(self.file.sheet_by_index(self.sheet))
                elif isinstance(sheet,str):
                    self.sheet.append(self.file.sheet_by_name(sheet))
                elif isinstance(sheet,list):
                    if isinstance(sheet[0],int):
                        self.sheet = [self.file.sheet_by_index(sheet_number) for sheet_number in sheet]
                    elif isinstance(sheet[0],str):
                        self.sheet = [self.file.sheet_by_name(sheet_name) for sheet_name in sheet]
                    else:
            ...
            # 读取数据
            self.data = [single_sheet.row_values(i) for single_sheet in self.sheet for i in range(single_sheet.nrows)]
